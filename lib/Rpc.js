...
Rpc._parseTxResult = function(info) {
  ...
  info.vout.forEach( function(o) {
    ...
    if (o.scriptPubKey.asm.indexOf("SSGEN") > -1 ) {
        ...
        } else if (config.network == 'hcdtestnet') {
...
Rpc.getBlock = function(hash, cb) {
  ...
  bitcoreRpc.getBlock(hash, function(err,info) {
    ...
    if (info.result.height) {
      ...
      // Set magic numbers from hcd/chaincfg/params.go
      switch (config.network) {
      case 'hcdlivenet':
        reductionInterval = 12288;
        base = 640000000;
        mulSubsidy = 999;
        divSubsidy = 1000;
        workProportion = 6;
      ...
      } else if ((info.result.height < 4096 && config.network === 'hcdlivenet') ||
                 (info.result.height < 775 && config.network === 'hcdtestnet')) {
